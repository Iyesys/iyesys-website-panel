...
export default function RemoveUserButton({ action }: { action: () => void }) {
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-6 shadow-lg">
            <h2 className="text-base font-semibold text-slate-900">Kullanıcıyı kaldır</h2>
            <p className="mt-2 text-sm text-slate-500">
              Bu kullanıcının panele erişimini kaldırmak istediğinize emin misiniz? Bu işlem geri alınamaz.
            </p>
